...
// min = 0 dB, max = 120 dB untuk tujuan visualisasi
...
class MainActivity : ComponentActivity() {
    companion object {
        ...
        private var isRunning = false   // menggunakan ini drpd MeterService.isRecording untuk menghindari balapan

        var coldStart = true

        fun dBToProgress(dB : Float) : Float {
            return dB/120 // scale dr  [0dB-120dB] sampai [0-1]
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON) // agar screen tetap nyala

        Log.d(TAG, "onCreate!")

        // Register panggilan balik yg memanggil metode finish() saat button kembali ditekan
        ...
        setContent {
            SoundMeterESPTheme {
                // Menggunakan tema bawaan device
                Surface(
    ...
    ) // jika pratinjau horizontal diinginkan, pratinjau OneSecView dan FiveMinView juga harus horizontal (atau tulis ulang kode untuk menangani kasus ini)
    @Composable
    fun AppContent(permissionState: PermissionState = FakePermissionState(PermissionStatus.Granted)) {
        val windowSizeClass = if(!LocalInspectionMode.current) calculateWindowSizeClass(this) else WindowSizeClass.calculateFromSize(DpSize(360.dp, 760.dp))   // fallback WindowSizeClass digunakan untuk pratinjau
    ...
    fun LeftFiveMinView(updateChartFiveLeft: Float, modifier: Modifier = Modifier){
        Row(modifier = modifier.padding(2.dp)) { // left
            Column(modifier = Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally) {  // susunan dalam kolom
                Text(text = "Left channel", fontWeight = FontWeight.Bold)
            ...
            confirmButton = if(!shouldShowRationale) {
                {
                    Button(onClick = {
                        ...
                        intent.data = uri
                        intent.putExtra(":settings:show_fragment_args", bundleOf(":settings:fragment_args_key" to "permission_settings"))   // sorot baris izin
    ...
    private class FakePermissionState(  // status izin palsu untuk digunakan dalam pratinjau
