...
object Values {
    ...
    // item baru ditambahkan di akhir, dan ketika ditampilkan dalam grafik, item tersebut muncul dari awal
    ...
    // elemen antrian muncul
    var lastLeft : Float = 0f
    var lastRight : Float = 0f

    // ketika item muncul dari antrian, item tersebut ditambahkan ke daftar nilai detik terakhir
    ...
    // menghitung jumlah item yang ditambahkan ke daftar detik terakhir
    private var leftCount = 0
    private var rightCount = 0

    // ketika daftar kedua terakhir penuh, daftar tersebut dikosongkan dan nilai maksimal ditambahkan ke daftar 5 menit terakhir
    ...
    fun updateQueues(leftMeasuredValues: FloatArray, rightMeasuredValues: FloatArray, readN: Int) {
        val leftValues = downsampleTo60Hz(leftMeasuredValues.sliceArray(0 until readN/2)).filter { it!=Float.NEGATIVE_INFINITY }     // turunkan sampel ke 60Hz (dari 44100Hz) dan ambil hanya bagian baca dari array
...
// Log.d(TAG, "getMaxDbLastSec: $maxLeft, $maxRight, ukuran: ${lastSecDbLeftList.size}, ${lastSecDbRightList.size}")
...
// Log.d(TAG, "downsampleTo60Hz: ukuran originalArray: ${originalArray.size} ukuran downsampledArray: ${downsampledArray.size}")
